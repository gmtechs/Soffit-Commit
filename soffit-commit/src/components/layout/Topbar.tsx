...
import { Search, Sun, Moon, X, ChevronDown, User, Link2, Bell } from "lucide-react";
...
export function Topbar() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (searchRef.current && !searchRef.current.contains(e.target as Node)) {
        setShowSearch(false); setQuery(""); setSearchResults([]); setPeerResults([]);
      }
  ...
  const handleSearch = async (q: string) => {
    setQuery(q);
    if (q.trim().length < 2) { setSearchResults([]); setPeerResults([]); setShowSearch(false); return; }
    try {
      ...
      setShowSearch(true);
      setSearchFocusIdx(-1);
    } catch { setSearchResults([]); setPeerResults([]); }
  };
    ...
    <header style={{
      ...
      <div ref={searchRef} style={{ position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, background: "var(--color-bg)", borderRadius: 6, padding: "7px 12px", width: 220, border: "1px solid var(--color-border)" }}>
          ...
          />
          {query && (
            <button onClick={() => { setQuery(""); setSearchResults([]); setShowSearch(false); }}
