...
pub async fn cmd_ai_chat_document(
    ...
    const CONTEXT_CHARS: usize = 18_000;
    let path = std::path::Path::new(&file_path);
    if !matches!(file_kind.as_str(), "excel" | "sql" | "csv" | "text" | "generic") {
        return Err("Document chat currently supports spreadsheets and text-based files.".to_string());
    }
    ...
    let app2 = app.clone();
    let result = tokio::task::spawn_blocking(move || {
        crate::ai::llm::complete(&model_path, &system, &question, None, move |token| {
