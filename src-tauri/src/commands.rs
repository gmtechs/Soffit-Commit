...
use crate::{
    activity, auth, locks, models::*, pairing, peers, shares, versions,
    excel, sql::engine::SqlEngine,
    ai::{model_manager::{ModelManager, AiStatus}, prompts, identity_guard},
    AppState,
};
...
#[tauri::command]
pub fn cmd_create_user(username: String, password: String, state: State<'_, AppState>) -> CmdResult<User> {
    let db = state.db.lock().map_err(e)?;
...
#[tauri::command]
pub fn cmd_login(username: String, password: String, state: State<'_, AppState>) -> CmdResult<User> {
    let db = state.db.lock().map_err(e)?;
...
#[tauri::command]
pub fn cmd_update_user(user_id: String, username: String, password: Option<String>, state: State<'_, AppState>) -> CmdResult<User> {
    let db = state.db.lock().map_err(e)?;
...
pub fn cmd_generate_pairing_code(state: State<'_, AppState>) -> CmdResult<PairingCodeWithQr> {
    ...
    let node_id = state.iroh_node.try_lock().ok()
        .and_then(|g| g.as_ref().map(|n| n.node_id.clone()));

    // Get the full EndpointAddr for embedding in the code
    let endpoint_addr_json = state.iroh_node.try_lock().ok()
        .and_then(|g| g.as_ref().map(|n| {
    ...
    let short_display = code.code.split(':').next().unwrap_or(&code.code).to_string();
...
#[tauri::command]
pub fn cmd_consume_pairing_code(code: String, display_name: String, state: State<'_, AppState>) -> CmdResult<Option<String>> {
    let db = state.db.lock().map_err(e)?;
    // Returns the embedded endpoint addr if present
    let endpoint_addr = pairing::consume_pairing_code(&db, &code).map_err(e)?;
    ...
    if let Some(ref addr_json) = endpoint_addr {
        if let Ok(node_guard) = state.iroh_node.try_lock() {
            if let Some(node) = node_guard.as_ref() {
                let node = node.clone();
                let addr = addr_json.clone();
                let name = display_name.clone();
                state.rt.spawn(async move {
                    if let Err(e) = node.connect_to_peer(&addr, &name).await {
                        eprintln!("[pairing] connect failed: {e}");
                    }
                });
            }
        }
...
#[tauri::command]
pub fn cmd_add_peer(node_id: String, display_name: String, public_key: String, endpoint_addr: Option<String>, state: State<'_, AppState>) -> CmdResult<Peer> {
    let db = state.db.lock().map_err(e)?;
    ...
    if let Ok(node_guard) = state.iroh_node.try_lock() {
        if let Some(node) = node_guard.as_ref() {
            for (node_id, addr_json) in reconnectable {
                let node = node.clone();
                state.rt.spawn(async move {
                    match node.connect_to_peer(&addr_json, "").await {
...
#[tauri::command]
pub fn cmd_rename_peer(peer_id: String, new_name: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    peers::rename_peer(&db, &peer_id, &new_name).map_err(e)
}

#[tauri::command]
pub fn cmd_set_permission(share_id: String, peer_id: String, level: String, state: State<'_, AppState>) -> CmdResult<SharePermission> {
    let db = state.db.lock().map_err(e)?;
    let perm = peers::set_permission(&db, &share_id, &peer_id, PermissionLevel::from(level.as_str())).map_err(e)?;
    activity::log_activity(&db, "system", "permission_changed", &share_id,
        Some(&format!("peer={peer_id} level={level}"))).ok();
    Ok(perm)
}
...
#[tauri::command]
pub fn cmd_create_share(path: String, display_name: String, state: State<'_, AppState>) -> CmdResult<Share> {
    let db = state.db.lock().map_err(e)?;
...
pub fn cmd_refresh_share(share_id: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    let share_path = db.query_row(
        "SELECT path FROM shares WHERE id = ?1",
        rusqlite::params![share_id], |row| row.get::<_, String>(0),
    ).map_err(e)?;
    shares::index_share_files(&db, &share_id, &share_path).map_err(e)
}

#[tauri::command]
pub fn cmd_set_selective_sync(share_id: String, enabled: bool, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
...
#[tauri::command]
pub fn cmd_acquire_lock(file_path: String, peer_id: String, peer_name: String, state: State<'_, AppState>) -> CmdResult<Lock> {
    let db = state.db.lock().map_err(e)?;
    ...
    if let Ok(node_guard) = state.iroh_node.try_lock() {
        if let Some(node) = node_guard.as_ref() {
            ...
            state.rt.spawn(async move { node.broadcast_lock(&fp, &pn).await.ok(); });
        }
    }
    Ok(lock)
}
...
#[tauri::command]
pub fn cmd_release_lock(file_path: String, peer_id: String, state: State<'_, AppState>) -> CmdResult<bool> {
    let db = state.db.lock().map_err(e)?;
    ...
    if released {
        ...
        if let Ok(node_guard) = state.iroh_node.try_lock() {
            if let Some(node) = node_guard.as_ref() {
                let node = node.clone();
                let fp = file_path.clone();
                state.rt.spawn(async move { node.broadcast_unlock(&fp).await.ok(); });
            }
        }
    }
    Ok(released)
...
#[tauri::command]
pub fn cmd_list_activity(limit: Option<usize>, state: State<'_, AppState>) -> CmdResult<Vec<ActivityEntry>> {
    let db = state.db.lock().map_err(e)?;
...
pub fn cmd_dashboard_stats(state: State<'_, AppState>) -> CmdResult<DashboardStats> {
    ...
    let storage_used_bytes: i64 = db.query_row(
        "SELECT COALESCE(SUM(size_bytes),0) FROM file_index", [], |r| r.get(0)).unwrap_or(0);
    let files_synced: i64 = db.query_row(
        "SELECT COUNT(*) FROM file_index WHERE sync_status='synced'", [], |r| r.get(0)).unwrap_or(0);

    let now = chrono::Utc::now();
    let week_start = (now - chrono::Duration::days(7)).to_rfc3339();
    let two_weeks_start = (now - chrono::Duration::days(14)).to_rfc3339();
    let month_start = format!("{}-{:02}-01T00:00:00Z",
        now.format("%Y"), now.format("%m").to_string().parse::<u32>().unwrap_or(1));

    let files_edited_this_month: i64 = db.query_row(
        "SELECT COUNT(*) FROM activity_log WHERE action='file_saved' AND timestamp>=?1",
        rusqlite::params![month_start], |r| r.get(0)).unwrap_or(0);
    let conflicts_resolved: i64 = db.query_row(
        "SELECT COUNT(*) FROM activity_log WHERE action='conflict_resolved'",
        [], |r| r.get(0)).unwrap_or(0);

    // Week-over-week deltas
    let files_this_week: i64 = db.query_row(
        "SELECT COUNT(*) FROM activity_log WHERE action='file_saved' AND timestamp>=?1",
        rusqlite::params![week_start], |r| r.get(0)).unwrap_or(0);
    ...
    let conflicts_this_week: i64 = db.query_row(
    ...
    let pct_delta = |cur: i64, prev: i64| -> i64 {
        if prev == 0 { if cur > 0 { 100 } else { 0 } }
        else { (cur - prev) * 100 / prev  }
    };
    ...
    let total_files: i64 = db.query_row(
        "SELECT COUNT(*) FROM file_index", [], |r| r.get(0)).unwrap_or(0).max(1);
    let sync_health_percent = (files_synced as f64 / total_files as f64) * 100.0;

    let excel_count: i64 = db.query_row(
        "SELECT COUNT(*) FROM file_index WHERE file_kind='excel'", [], |r| r.get(0)).unwrap_or(0);
    let sql_count: i64 = db.query_row(
        "SELECT COUNT(*) FROM file_index WHERE file_kind='sql'", [], |r| r.get(0)).unwrap_or(0);
    let online_peers: i64 = db.query_row(
        "SELECT COUNT(*) FROM peers WHERE is_online=1", [], |r| r.get(0)).unwrap_or(0);
    let total_peers: i64 = db.query_row(
        "SELECT COUNT(*) FROM peers", [], |r| r.get(0)).unwrap_or(0);

    Ok(DashboardStats {
        storage_used_bytes, files_synced, files_edited_this_month, conflicts_resolved,
        sync_health_percent,
        file_type_breakdown: FileTypeBreakdown {
            excel: excel_count, sql: sql_count,
            other: (total_files - excel_count - sql_count).max(0),
        },
        online_peers, total_peers,
...
pub fn cmd_get_setting(key: String, state: State<'_, AppState>) -> CmdResult<Option<String>> {
    let db = state.db.lock().map_err(e)?;
    match db.query_row("SELECT value FROM app_settings WHERE key=?1",
...
pub fn cmd_excel_set_cell(
    path: String, sheet_index: usize, row: u32, col: u32, value: String,
...
#[tauri::command]
pub fn cmd_excel_get_record(path: String, layout: excel::FormLayout, record_index: u32) -> CmdResult<excel::FormRecord> {
    excel::get_record(std::path::Path::new(&path), &layout, record_index).map_err(e)
}

#[tauri::command]
pub fn cmd_excel_save_record(
    path: String, layout: excel::FormLayout, record_index: u32,
    updates: Vec<(u32, String)>, state: State<'_, AppState>,
...
#[tauri::command]
pub fn cmd_sql_execute(sql: String, state: State<'_, AppState>) -> CmdResult<crate::sql::engine::QueryResult> {
    let engine = state.sql_engine.lock().map_err(e)?;
    Ok(engine.execute(&sql))
}

#[tauri::command]
pub fn cmd_sql_execute_script(sql: String, state: State<'_, AppState>) -> CmdResult<Vec<crate::sql::engine::ScriptStatementResult>> {
    let engine = state.sql_engine.lock().map_err(e)?;
    Ok(engine.execute_script(&sql))
}

#[tauri::command]
pub fn cmd_sql_import_dump(file_path: String, state: State<'_, AppState>) -> CmdResult<crate::sql::engine::ImportResult> {
    let engine = state.sql_engine.lock().map_err(e)?;
    engine.import_sql_dump(std::path::Path::new(&file_path)).map_err(e)
}

#[tauri::command]
pub fn cmd_sql_query_file(file_path: String, sql: String, state: State<'_, AppState>) -> CmdResult<crate::sql::engine::QueryResult> {
    let engine = state.sql_engine.lock().map_err(e)?;
    Ok(engine.query_file(std::path::Path::new(&file_path), &sql))
}

#[tauri::command]
pub fn cmd_sql_attach_file(file_path: String, alias: String, state: State<'_, AppState>) -> CmdResult<()> {
    let engine = state.sql_engine.lock().map_err(e)?;
    engine.attach_file(std::path::Path::new(&file_path), &alias).map_err(e)
}

#[tauri::command]
pub fn cmd_sql_generate_edit(
    table: String, pk_col: String,
    pk_val: serde_json::Value, changes: Vec<(String, serde_json::Value)>,
) -> CmdResult<String> {
    Ok(SqlEngine::generate_edit_sql(&table, &pk_col, &pk_val, &changes))
}

// ── Version history ───────────────────────────────────────────────────────────

#[tauri::command]
pub fn cmd_list_versions(file_path: String, state: State<'_, AppState>) -> CmdResult<Vec<versions::Version>> {
    let db = state.db.lock().map_err(e)?;
...
#[tauri::command]
pub fn cmd_list_conflicts(state: State<'_, AppState>) -> CmdResult<Vec<crate::conflicts::Conflict>> {
    let db = state.db.lock().map_err(e)?;
    crate::conflicts::list_conflicts(&db).map_err(e)
}

#[tauri::command]
pub fn cmd_resolve_conflict(conflict_id: String, resolution: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    let (file_path, _): (String, String) = db.query_row(
        "SELECT file_path, remote_hash FROM conflicts WHERE id=?1",
        rusqlite::params![conflict_id], |row| Ok((row.get(0)?, row.get(1)?)),
    ).map_err(e)?;
    ...
    crate::conflicts::resolve_conflict(&db, &conflict_id, res,
        std::path::Path::new(&file_path), None).map_err(e)?;
    activity::log_activity(&db, "local", "conflict_resolved", &file_path, Some(&resolution)).ok();
    Ok(())
}
...
#[tauri::command]
pub fn cmd_search(query: String, limit: Option<usize>, state: State<'_, AppState>) -> CmdResult<Vec<crate::search::SearchResult>> {
    let db = state.db.lock().map_err(e)?;
    crate::search::search(&db, &query, limit.unwrap_or(20)).map_err(e)
}

#[tauri::command]
pub fn cmd_index_file(share_id: String, rel_path: String, abs_path: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
...
#[tauri::command]
pub fn cmd_record_file_open(file_path: String, file_kind: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    let id = uuid::Uuid::new_v4().to_string();
    let now = chrono::Utc::now().to_rfc3339();
    // Keep last 50 entries per file — delete older ones
    db.execute("DELETE FROM file_history WHERE file_path = ?1", rusqlite::params![file_path]).ok();
    db.execute(
...
#[tauri::command]
pub fn cmd_get_file_history(limit: Option<usize>, state: State<'_, AppState>) -> CmdResult<Vec<FileHistoryEntry>> {
    let db = state.db.lock().map_err(e)?;
    ...
    let items = stmt.query_map(rusqlite::params![lim], |row| {
        Ok(FileHistoryEntry { id: row.get(0)?, file_path: row.get(1)?, opened_at: row.get(2)?, file_kind: row.get(3)? })
    }).map_err(e)?.filter_map(|r| r.ok()).collect();
    Ok(items)
}

#[tauri::command]
pub fn cmd_add_favourite(file_path: String, display_name: String, file_kind: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    let id = uuid::Uuid::new_v4().to_string();
    let now = chrono::Utc::now().to_rfc3339();
    db.execute(
...
pub fn cmd_remove_favourite(file_path: String, state: State<'_, AppState>) -> CmdResult<()> {
    let db = state.db.lock().map_err(e)?;
    db.execute("DELETE FROM file_favourites WHERE file_path = ?1", rusqlite::params![file_path]).map_err(e)?;
    Ok(())
}
...
pub fn cmd_get_favourites(state: State<'_, AppState>) -> CmdResult<Vec<FileFavourite>> {
    ...
    let items = stmt.query_map([], |row| {
        Ok(FileFavourite { id: row.get(0)?, file_path: row.get(1)?, display_name: row.get(2)?, file_kind: row.get(3)?, added_at: row.get(4)? })
    }).map_err(e)?.filter_map(|r| r.ok()).collect();
    Ok(items)
}
...
#[tauri::command]
pub fn cmd_write_file_bytes(file_path: String, data: Vec<u8>, state: State<'_, AppState>) -> CmdResult<()> {
...
pub fn cmd_ai_status(state: State<'_, AppState>) -> CmdResult<AiStatus> {
    let mgr = ModelManager::new(&state.versions_dir.lock().map_err(e)?.parent().unwrap_or(std::path::Path::new(".")));
    Ok(mgr.status())
}

/// Start downloading a model. Emits "ai-download-progress" events: {filename, bytes_done, bytes_total}
#[tauri::command]
pub async fn cmd_ai_download_models(app: tauri::AppHandle, state: tauri::State<'_, AppState>) -> CmdResult<()> {
    let data_dir = state.versions_dir.lock().map_err(e)?.parent().unwrap_or(std::path::Path::new(".")).to_path_buf();
    ...
    for (url, filename, sha256) in [
        (crate::ai::model_manager::CHAT_MODEL_URL, crate::ai::model_manager::CHAT_MODEL_FILENAME, crate::ai::model_manager::CHAT_MODEL_SHA256),
        (crate::ai::model_manager::EMBED_MODEL_URL, crate::ai::model_manager::EMBED_MODEL_FILENAME, crate::ai::model_manager::EMBED_MODEL_SHA256),
    ] {
        ...
        mgr.download(url, filename, sha256, move |done, total| {
            app2.emit("ai-download-progress", serde_json::json!({
...
pub async fn cmd_ai_explain_conflict(
    file_path: String, local_meta: String, remote_meta: String, diff_snippet: Option<String>,
    app: tauri::AppHandle, state: tauri::State<'_, AppState>,
) -> CmdResult<AiTextResult> {
    let model_path = ModelManager::new(
        &state.versions_dir.lock().map_err(e)?.parent().unwrap_or(std::path::Path::new(".")).to_path_buf()
    ).chat_model_path();
    drop(state); // release State before await
    let system = prompts::conflict_explain(&file_path, &local_meta, &remote_meta, diff_snippet.as_deref());
    let app2 = app.clone();
    let result = tokio::task::spawn_blocking(move || {
        crate::ai::llm::complete(&model_path, &system, "Explain this conflict.", None, move |tok| {
            app2.emit("ai-token", tok).ok(); true
        })
    }).await.map_err(e)?.map_err(e)?;
    let guarded = result == identity_guard::canned();
    Ok(AiTextResult { text: result, guarded })
}

/// 6.4 Activity summary
#[tauri::command]
pub async fn cmd_ai_activity_summary(
    time_window: String, app: tauri::AppHandle, state: tauri::State<'_, AppState>,
) -> CmdResult<AiTextResult> {
...
pub async fn cmd_ai_data_insights(
    stats_json: String, app: tauri::AppHandle, state: tauri::State<'_, AppState>,
) -> CmdResult<AiTextResult> {
    let model_path = ModelManager::new(
        &state.versions_dir.lock().map_err(e)?.parent().unwrap_or(std::path::Path::new(".")).to_path_buf()
    ).chat_model_path();
    ...
    let app2 = app.clone();
    let result = tokio::task::spawn_blocking(move || {
        crate::ai::llm::complete(&model_path, &system, "Describe these stats.", None, move |tok| {
            app2.emit("ai-token", tok).ok(); true
        })
    }).await.map_err(e)?.map_err(e)?;
    let guarded = result == identity_guard::canned();
...
pub async fn cmd_ai_chat_document(
    file_path: String, file_kind: String, question: String, document_text: Option<String>,
    app: tauri::AppHandle, state: tauri::State<'_, AppState>,
) -> CmdResult<AiTextResult> {
    ...
    let path = std::path::Path::new(&file_path);
    if !matches!(file_kind.as_str(), "excel" | "sql" | "csv" | "text" | "generic" | "pdf") {
        return Err("Document chat currently supports PDFs, spreadsheets, and text-based files.".to_string());
    }
    let excerpt = if let Some(text) = document_text.filter(|text| !text.trim().is_empty()) {
        ...
        for sheet in sheets.iter().take(3) {
            ...
            for row in data.rows.iter().take(100) {
                ...
                if !values.trim().is_empty() { text.push_str(&values); text.push('\n'); }
                if text.len() >= CONTEXT_CHARS { break; }
            }
            if text.len() >= CONTEXT_CHARS { break; }
        }
        text
        ...
        pdf_extract::extract_text(path)
            .map_err(|_| "This PDF has no extractable text. If it is a scan, OCR it first.".to_string())?
    } else {
    ...
    if excerpt.trim().is_empty() { return Err(e(anyhow::anyhow!("This document has no readable text."))); }
    let model_path = ModelManager::new(
        &state.versions_dir.lock().map_err(e)?.parent().unwrap_or(std::path::Path::new(".")).to_path_buf()
    ).chat_model_path();
    drop(state);
    ...
    let result = tokio::task::spawn_blocking(move || {
        crate::ai::llm::complete_document(&model_path, &system, &question, move |token| {
            app2.emit("ai-token", token).ok(); true
        })
    }).await.map_err(e)?.map_err(e)?;
    let guarded = result == identity_guard::canned();
    Ok(AiTextResult { text: result, guarded })
...
fn document_context(document: &str, question: &str, max_chars: usize) -> String {
    if document.len() <= max_chars { return document.to_string(); }
    ...
    if !current.is_empty() { text_chunks.push(current); }
    let mut chunks: Vec<(usize, usize, String)> = text_chunks.into_iter()
        .enumerate()
        .map(|(index, chunk)| {
    ...
    for (_, _, chunk) in chunks {
        ...
        if used + chunk_len > max_chars { continue; }
        selected.push(chunk);
        used += chunk_len;
